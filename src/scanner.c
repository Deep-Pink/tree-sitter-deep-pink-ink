...
// Typedef for numeric block level; juuust in case we want to change the amount
// of nesting we allow. To future self … CAUTION: This needs to be serialized to
// a string of bytes, so account for that when changing it to something larger.
...
unsigned tree_sitter_deep_pink_ink_external_scanner_serialize(void *payload,
  ...
  if (size >= TREE_SITTER_SERIALIZATION_BUFFER_SIZE) {
    // printf needs to be commented out to be compiled for wasm (i.e. to use the
    // playground). printf("WARN: Bumped up against tree sitter serialization
    // limit (%d)! We may have lost data!\n",
...
void tree_sitter_deep_pink_ink_external_scanner_deserialize(void *payload,
  ...
  if (buffer != NULL && length > 0) {
    ...
    scanner->is_in_command = buffer[size++] > 0 ? true : false;
  }
}
